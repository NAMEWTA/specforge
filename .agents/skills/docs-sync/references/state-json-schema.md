# state.json Schema（docs-sync 持久化状态）

存储位置：`.agents/skills/docs-sync/state.json`

该文件是 docs-sync 技能的**唯一**持久化状态。每次成功同步后原子化写入。必须跟随仓库提交到 git（作为同步基线的事实来源）。
...
| `skill` | string | ✓ | 固定为 `"docs-sync"`，便于多技能共用 state 目录时辨识 |
| `last_sync_sha` | string (40 hex) | ✓ | 上次成功同步后记录的 commit SHA；下次 diff 的起点 |
| `last_sync_short` | string (7 hex) | ✓ | `last_sync_sha` 的前 7 位，便于日志阅读 |
| `last_sync_commit_subject` | string | ✓ | 上次基线 commit 的 subject 行 |
| `last_sync_commit_date` | string (ISO 8601) | ✓ | 上次基线 commit 的 author date |
| `last_sync_run_at` | string (ISO 8601 UTC) | ✓ | 本次同步**完成写入**时刻；与 `last_sync_commit_date` 不同 —— 前者是"我什么时候做的"，后者是"基线 commit 什么时候创建的" |
...
```json
{
  ...
  "last_sync_commit_subject": "feat(templates): enrich command-creator & add github-ops skill (v0.0.3)",
...
```json
{
  ...
  "total_syncs": 3,
  "synced_docs": ["README.md", "CHANGELOGS.md"]
}
```
...
1. **原子化**：先写 `state.json.tmp`，再 `mv state.json.tmp state.json`。直接覆盖可能导致中断时 state 被截断
...
读取 state.json 时按以下降级链处理：

1. 文件不存在 → 首次运行流程（创建新文件，基线设为当前 HEAD）
2. JSON 解析失败 → 报错退出，要求用户手动修复或删除
3. `schema_version > 1` → 报错退出，提示技能版本过旧
4. `last_sync_sha` 不是 40 字符十六进制 → 视为损坏，回退到首次运行
...
- state.json **提交到 git**，作为仓库的一部分
- `last_sync_sha` 引用的 commit 必须在当前分支的历史中
- 如果多人协作导致 state.json 冲突：保留 `last_sync_sha` 值较新（即在 git 历史中更靠后）的那个，`total_syncs` 取较大值，`previous_sync_sha` 取被保留一方的值

## 不要做的事

- 不要把 state.json 放进 `.gitignore`
